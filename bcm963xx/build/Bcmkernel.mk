...
export KERNEL_DEBUG
export BUILD_BMU BUILD_PHY_ADSL   
export ORIG_PROFILE_ARCH
export BUILD_BCM_WLAN_NO_MFGBIN

.PHONY: default version_info headers_install oldnoconfig

default: version_info
bcm_vmlinux: version_info
dtbs: bcm_vmlinux
version_info: olddefconfig
headers_install: modules_install
modules_install: modules
modules: default bcm_vmlinux
mrproper: clean tools/perf_clean
tools/perf_clean: clean
oldnoconfig: olddefconfig

# Dummy target for iopsys-do-kernel
.PHONY: iopsys-do-kernel
iopsys-do-kernel:
	@echo "iopsys-do-kernel target is not needed, skipping..."

ifeq ($(strip $(DESKTOP_LINUX)),)
ifeq ($(strip $(BCA_CPEROUTER)),y)
version_info olddefconfig bcm_vmlinux modules modules_install dtbs:
else
olddefconfig bcm_vmlinux modules modules_install dtbs:
endif
	@if [ ! -f "$@" ] || find -name "iopsys-do-kernel" ! -newer "$@" | \
		xargs -r false; then \
		echo "doing kernel build for $@" && \
		$(MAKE) -C $(KERNEL_DIR) $(MAKEOVERRIDES) "$@" && \
		touch "$@"; \
	else \
		echo "kernel build for $@ already done, skipping"; \
	fi
else
...
.PHONY: clean mrproper tools/perf tools/perf_clean
clean mrproper tools/perf tools/perf_clean:
	$(MAKE) -C $(KERNEL_DIR) $(MAKEOVERRIDES) $(MAKECMDGOALS) $@
	rm -f olddefconfig bcm_vmlinux modules modules_install dtbs iopsys-do-kernel
