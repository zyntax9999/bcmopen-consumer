...
/* ---- Public Variables ------------------------------------------------- */
/* Provided by linker-script to enable extending with parms from other files */
extern bp_elem_t * g_VoiceBoardParms[];
...
static bp_elem_t g_voice_arcadyan[] = {
    {bp_cpBoardId,               .u.cp = "963158REF_ARC"},
    {bp_usFxsFxo1SpiSSNum,       .u.us  = SPI_DEV_1},
    {bp_usGpioFxsFxoRst1,        .u.us  = BP_GPIO_4_AL},
    {bp_usGpioFxsFxoRst2,        .u.us  = BP_GPIO_27_AL}, /* DECT Reset */
    {bp_daughterCardList,        .u.ptr = g_963158_dCardList},
    {bp_last}
};

bp_elem_t * brcm_g_VoiceBoardParms[] SECTION_BRCM_G_VOICEBOARDPARMS =
{
...
int BpGetVoiceDectType( const char *pszBaseBoardId )
{
   ...
   // Inteno external Dect? Search for the voice daughter pointer.
   if(BpGetNumVoiceBoardIds(pszBaseBoardId)) {
      int i, j;

      for(i = 0; pBpStartElem[i].id != bp_daughterCardList &&
         pBpStartElem[i].id != bp_last; i++);

      if(pBpStartElem[i].id != bp_last) {
         VOICE_DAUGHTER_BOARD_PARMS **daughterCardList = pBpStartElem[i].u.ptr;
