...
extern int iopsys_setup_mtd_parts(struct mtd_info* mtd);
...
static int __init mtd_init(void)
{
	...
	/*
	 * Unlike setup_mtd_parts(), iopsys_setup_mtd_parts() doesn't call kerSysNvRamLoad()
	 * so we follow the same approch and disable the call here. However it appears there is no actual difference,
	 * the nvram was properly loaded in either cases.
	 */
#if 0
        kerSysNvRamLoad(mtd);
#endif

	iopsys_setup_mtd_parts(mtd);
    }
