#!/bin/bash
# This scripts is to build the DESKTOP_LINUX/DESKTOP_BEEP
...
if [ "$checkresult_build" != "1" ]; then
  ...
  if [ "$cms_enabled" == "1" ]; then
    ...
    make -C userspace private/apps/tr64
  ...
  make -C userspace public/apps/psictl
...
if [ "$beep_enabled" == "1" ]; then
  ...
  if [ -f userspace/gpl/apps/lxc/lxc-2.0.4/src/lxc/lxc-attach ]; then
...
if [ "$cms_enabled" == "1" ]; then
  if [ "$beep_enabled" == "1" ]; then
    bee_dirs="pmd dmad cwmpd dad bbcd cwmpctl firewalld"
    ...
    if [ -f userspace/private/apps/spTestSuite/spMaster ] && [ -f userspace/private/apps/spTestSuite/spRobot ] && [ -f userspace/private/apps/spTestSuite/spSuperRobot ] ; then
...
if [ "$cms_enabled" == "1" ]; then
  ...
  if $(grep -q "BUILD_IPPD=dynamic"  ./targets/$pr/$pr) ; then
    if [ -f userspace/private/apps/ippd/ippd ]; then
...
if $(grep -q "BUILD_SSHD=y"  ./targets/$pr/$pr) ; then
  if [ -f userspace/public/apps/dropbear/dropbear-2019.78/dropbear ]; then
