...
static int easysoc_nl_enable = 0;
...
static const struct genl_multicast_group netlink_event_mcgrps[] = {
		 { .name = "notify", },
};

static struct genl_family easysoc_nl_family = {
		 .id = GENL_ID_GENERATE,
		 .name = "easysoc",
		 .version = 1,
		 .maxattr = NL_MAX,
		 .mcgrps = netlink_event_mcgrps,
		 .n_mcgrps = ARRAY_SIZE(netlink_event_mcgrps),
};

void easysoc_netlink_init(void)
{
	int rc;

	rc = genl_register_family(&easysoc_nl_family);
	printk("netlink init\n");
	if (rc != 0 )
...
void __exit easysoc_netlink_exit(void)
{
	if(easysoc_nl_enable)
		genl_unregister_family(&easysoc_nl_family);
}

void easysoc_netlink_sendmsg(const char *s)
{
	struct sk_buff *skb;
	int rc;
	void *msg_head;
	...
	msg_head = genlmsg_put(skb, 0, seq++, &easysoc_nl_family, 0, NL_MSG);
	if (msg_head == NULL) {
		rc = -ENOMEM;
	...
	rc = nla_put_string(skb, NL_MSG, msg_buf);
	if (rc != 0){
	...
	rc = genlmsg_multicast_allns(&easysoc_nl_family, skb, 0, 0, GFP_ATOMIC);
	if (rc != 0) {
