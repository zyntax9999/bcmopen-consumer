...
static int rdpa_user_vlan_options_get(rdpa_ioctl_cmd_t *pa)
{
	bdmf_number parm;

	BDMF_TRACE_DBG("inside vlan_user_options_get\n");

	if ((pa->ret = rdpa_vlan_options_get(pa->mo, &parm)))
	{
		BDMF_TRACE_DBG("rdpa_vlan_options_get failed, ret:%d\n", pa->ret);
	}
...
static int rdpa_user_vlan_options_set(rdpa_ioctl_cmd_t *pa)
{

	BDMF_TRACE_DBG("inside vlan_user_options_set\n");

	if ((pa->ret = rdpa_vlan_options_set(pa->mo, (bdmf_number)(long)pa->parm)))
	{
		BDMF_TRACE_DBG("rdpa_vlan_options_set failed, ret:%d\n", pa->ret);
	}
...
long rdpa_vlan_ag_ioctl(unsigned int op, rdpa_ioctl_cmd_t *pa)
{
	...
	switch (op){
		...
		case RDPA_VLAN_OPTIONS_GET:
			ret = rdpa_user_vlan_options_get(pa);
			break;
