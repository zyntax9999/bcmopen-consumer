...
bdmf_error_t ag_drv_natc_eng_command_status_set(uint8_t eng_idx, const natc_eng_command_status *command_status)
{
    ...
    if((eng_idx >= BLOCK_ADDR_COUNT) ||
       ...
       (command_status->unused10 >= _1BITS_MAX_VAL_) ||
    ...
    reg_command_status = RU_FIELD_SET(eng_idx, NATC_ENG, COMMAND_STATUS, UNUSED10, reg_command_status, command_status->unused10);
...
bdmf_error_t ag_drv_natc_eng_command_status_get(uint8_t eng_idx, natc_eng_command_status *command_status)
{
    ...
    command_status->unused10 = RU_FIELD_GET(eng_idx, NATC_ENG, COMMAND_STATUS, UNUSED10, reg_command_status);
...
int bcm_natc_eng_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_natc_eng_command_status:
    {
        ...
        bdmf_session_print(session, "unused10 = %u (0x%x)\n", command_status.unused10, command_status.unused10);
...
static int bcm_natc_eng_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        natc_eng_command_status command_status = {.del_cmd_ddr_bin=gtmv(m, 8), .del_cmd_mode=gtmv(m, 1), .cache_flush=gtmv(m, 1), .decr_count=gtmv(m, 1), .nat_tbl=gtmv(m, 3), .multihash_count=gtmv(m, 4), .cache_hit=gtmv(m, 1), .miss=gtmv(m, 1), .error=gtmv(m, 1), .busy=gtmv(m, 1), .unused10=gtmv(m, 1), .command=gtmv(m, 3)};
        if(!err) bdmf_session_print(session, "ag_drv_natc_eng_command_status_set(%u %u %u %u %u %u %u %u %u %u %u %u %u)\n", eng_idx, command_status.del_cmd_ddr_bin, command_status.del_cmd_mode, command_status.cache_flush, command_status.decr_count, command_status.nat_tbl, command_status.multihash_count, command_status.cache_hit, command_status.miss, command_status.error, command_status.busy, command_status.unused10, command_status.command);
        ...
        if(err || command_status.del_cmd_ddr_bin!=gtmv(m, 8) || command_status.del_cmd_mode!=gtmv(m, 1) || command_status.cache_flush!=gtmv(m, 1) || command_status.decr_count!=gtmv(m, 1) || command_status.nat_tbl!=gtmv(m, 3) || command_status.multihash_count!=gtmv(m, 4) || command_status.cache_hit!=gtmv(m, 1) || command_status.miss!=gtmv(m, 1) || command_status.error!=gtmv(m, 1) || command_status.busy!=gtmv(m, 1) || command_status.unused10!=gtmv(m, 1) || command_status.command!=gtmv(m, 3))
...
bdmfmon_handle_t ag_drv_natc_eng_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        static bdmfmon_cmd_parm_t set_command_status[]={
            ...
            BDMFMON_MAKE_PARM("unused10", "unused10", BDMFMON_PARM_NUMBER, 0),
