...
extern int runner_core_size[runner_image_last + 1];
#endif
...
void drv_rnr_load_microcode(void)
{
	...
	uint32_t mem_inst_byte_num;

    for (rnr_idx = 0; rnr_idx < _num_of_cores; rnr_idx++)
    {
        mem_inst_byte_num = drv_rnr_get_inst_word_size(rnr_idx) * sizeof(uint32_t);
        MWRITE_BLK_32_SWAP(DEVICE_ADDRESS(RU_BLK(RNR_INST).addr[rnr_idx] + RU_REG_OFFSET(RNR_INST, MEM_ENTRY)),
            (uint8_t *)fw_inst_binaries[rnr_idx], mem_inst_byte_num);
    }
#else
    uint32_t inst;
    uint32_t mem_inst_word_num;
#ifdef CONFIG_BRCM_IKOS
    uint8_t nops;
#endif
    for (rnr_idx = 0; rnr_idx < _num_of_cores; rnr_idx++)
    {
        mem_inst_word_num = drv_rnr_get_inst_word_size(rnr_idx);
#ifdef CONFIG_BRCM_IKOS
    	nops = 0;
#endif
...
{
#if !defined(DUAL_ISSUE) || !defined(XRDP_EMULATION)
   uint8_t *src = (uint8_t *)__from;
    ...
    uint8_t *valToWriteP = (uint8_t *)(&valToWrite);

    for (i = 0; i < (__n / 4); i++, src += 2, dst += 4)
    {
        valToWriteP[0] = src[0];
        MWRITE_32(dst, (unsigned int)(*(volatile unsigned short *)valToWriteP));
        valToWriteP[0] = src[1];
        dst += 4;
        MWRITE_32(dst, (unsigned int)(*(volatile unsigned short *)valToWriteP));
    }
...
#if defined(DUAL_ISSUE)
		 BDMFMON_MAKE_PARM_RANGE("address_mode", "0: No active, 1: run in address range", BDMFMON_PARM_UNUMBER, 0, 0, 1),
		 BDMFMON_MAKE_PARM_RANGE("address_min", "in case address range mode, minimum address to record ", BDMFMON_PARM_UNUMBER, 0, 0, 0x0fffffff),
		 BDMFMON_MAKE_PARM_RANGE("address_max", "in case address range mode, minimum address to record ", BDMFMON_PARM_UNUMBER, 0, 0, 0x0fffffff),
#endif
